...
export const TournamentSchema = registry.register(
    ...
    z.object({
        ...
        status: z
            .nativeEnum(TournamentStatus, { message: 'Status must be a string' })
            .optional()
        ...
        game: z.number({ message: 'Game must be a number representing a game id' }).openapi({ example: 5 }),
        ...
        region: z
            .number({ message: 'Region must be a number representing a region id' })
            .optional()
            .openapi({ example: 15 }),
        ...
        tags: z.array(z.number()).openapi({ example: [1, 2, 5] }),
